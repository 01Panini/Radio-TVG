...
const VideoTab = () => {
  ...
  const openVideo = (video: ActiveVideo) => {
  ...
  const liveVideo: ActiveVideo | null = isLive && liveStreamUrl ? {
    id: 'live', title: 'Transmissão ao Vivo',
    thumbnail: '', isLive: true, hlsSrc: liveStreamUrl,
  } : null;
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="min-h-screen pb-24">
        ...
        {liveVideo && (
          <motion.div whileTap={{ scale: 0.98 }} onClick={() => openVideo(liveVideo)}
            ...
            <div className="aspect-video bg-gradient-to-br from-destructive/20 to-destructive/5">
              <div className="absolute inset-0 flex items-center justify-center">
                <Radio className="h-12 w-12 text-live/30" />
              </div>
            </div>
              ...
              <div className="flex flex-col gap-3">
                {videos.slice(3).map((video, i) => (
                  <motion.div key={video.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="relative flex-shrink-0 w-[160px] rounded-xl overflow-hidden bg-muted/30">
